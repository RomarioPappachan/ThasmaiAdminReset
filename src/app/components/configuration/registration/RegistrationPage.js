...
function RegistrationPage({ dummyData }) {
  console.log(dummyData);
  ...
  function handleOnChange(key, value) {
    setInputFieldValue(prev => ({
      ...prev,
      [key]: value // Dynamically update field or value
    }));
  }
  ...
  async function handleSubmit() {
    try {
      if (inputFieldValue.field && inputFieldValue.value) {
        ...
        if (response) {
          setRenderTableToggle(prevValue => !prevValue); // Trigger re-render to fetch updated data
        }
      } else {
        toast("Please enter values for both the field and value.");
      }
    } catch (error) {
      console.log("Error updating app configuration", error);
      toast.error("Error updating parameter.");
    }
                ...
                <td className="text-left ps-5">
                  {editableId === data.id ? (
                    <input
                      className="w-full h-[35px] px-2 border-none bg-[#D9D9D9] rounded"
                      type="text"
                      value={inputFieldValue.field} // Set current field value
                      onChange={(e) => handleOnChange("field", e.target.value)} // Update field value
                    />
                  ) : (
                    data.field
                  )}
                ...
                <td className="text-center text-indigo-600">
                  {editableId === data.id ? (
                    <input
                      className="w-full h-[35px] px-2 border-none bg-[#D9D9D9] rounded"
                      type="text"
                      value={inputFieldValue.value} // Set current value
                      onChange={(e) => handleOnChange("value", e.target.value)} // Update value
                    />
                  ) : (
                    data.value
                  )}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setInputFieldValue({
                          field: data.field,
                          value: data.value
                        }); // Set current field and value when Edit is clicked
                      }}
                    >
                      Edit
                    </button>
